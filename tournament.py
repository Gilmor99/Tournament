...
def playerStandings():
    ...
    DB = connect()
    cursor = DB.cursor()
    cursor.execute("select player_id, player_name, ranked, played from ranking order by ranked desc, played desc")
    standing = [row for row in cursor.fetchall()]
    # standing = [{int(row[0]), str(row[1]), int(row[2]), int(row[3])}
